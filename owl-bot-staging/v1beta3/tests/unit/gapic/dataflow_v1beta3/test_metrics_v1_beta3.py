...
async def test_get_job_execution_details_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.get_job_execution_details(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_get_stage_execution_details_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.get_stage_execution_details(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
